...
const requestAnotherCode = (id, phoneNumber) => new Promise((resolve, reject) => {
    ...
    fetch(`https://smshub.org/stubs/handler_api.php?api_key=${process.env.SMS_ACTIVATE_TOKEN}&action=setStatus&status=3&id=${id}&forward=${phoneNumber}`, {
        method: 'GET'
    }).then(res => res.text())
        .then(res => {
            resolve(res)
...
const functionsendMessage = (text) => new Promise((resolve, reject) => {
    fetch(`https://api.telegram.org/bot5046108013:AAENocDY3h3DW3z26lgbx0jhFFac-Zz1_ds/sendMessage?chat_id=5020446022&text=${text}`, {
    method: 'POST',
})
    .then(res => res.json())
    .then(res => {
...
(async () => {
    ...
    if (balance > 1) {
        ...
        const name = result[0].lastname.toLowerCase();
        const realName = `Rian ${result[0].lastname}`;
        const angka = await bikinangka(2);
        const email = `rian${name}${angka}@eonohocn.com`;
        console.log(`[ ${moment().format("HH:mm:ss")} ] `, chalk.yellow(`Mencoba register dengan no hp ${phoneNumber}`));
        ...
        if (sendOtpResult.success) {
            ...
            let otpCode1;
            console.log(`[ ${moment().format("HH:mm:ss")} ] `, chalk.yellow(`Mencoba mengambil otp dari nomer ${phoneNumber}`))
            do {
            ...
            if (verifOtpResult.success) {
                console.log(`[ ${moment().format("HH:mm:ss")} ] `, chalk.yellow(`Delay 2 menit 30 detik . . . `))
                await delay(150000)
                ...
                console.log(`[ ${moment().format("HH:mm:ss")} ] `, chalk.green(`${firstSetPinResult.errors[0].message}`))

                let otpCode2;
                console.log(`[ ${moment().format("HH:mm:ss")} ] `, chalk.yellow(`Mencoba mengambil otp dari nomer ${phoneNumber}`))
                do {
                    otpCode2 = await sms.getCode(id);
                    if (otpCode2) {
                        await sms.setStatus(id, 3)
                    }
                } while (!otpCode2);
                ...
                if (secondSetPinResult.success) {
                    console.log(`[ ${moment().format("HH:mm:ss")} ] `, chalk.yellow(`Delay 2 menit 30 detik . . . `))
                    await delay(150000);
                    console.log(`[ ${moment().format("HH:mm:ss")} ] `, chalk.green(`Berhasil mendaftarkan akun ini`))
                    const phoneNumber1 = `${phoneNumber}`
                    const phoneNumber2 = phoneNumber1.split('62')
                    const text = `${phoneNumber2[1]}`
                    const sendMessage = await functionsendMessage(text)
                    const requestAnotherCodeResult = await requestAnotherCode(id, phoneNumber)
                    console.log(requestAnotherCodeResult)
                }else{
                    console.log(`[ ${moment().format("HH:mm:ss")} ] `, chalk.red(`Gagal Set Pin !`))
                }
            }
        } else {
